Client.socket.messages.register("OnRoomEntityAdd", async function(data) {
    if(data.furnitures != undefined) {
        ...
        for(let index in data.furnitures) {
            ...
            let entity = new RoomItemFurniture(Client.rooms.interface.entity, dataFurniture.furniture, dataFurniture.position.direction);
    ...
    if(data.users != undefined) {
        ...
        for(let index in data.users) {
            let entity = new RoomItemFigure(Client.rooms.interface.entity, data.users[index].figure, data.users[index].position.direction);

            entity.on("ready", async () => {
